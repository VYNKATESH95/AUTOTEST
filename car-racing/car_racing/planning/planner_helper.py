...
def debug_plot(track, vehicles, target_traj_xglob):
    fig, ax = plt.subplots()
    track.plot_track(ax)
    x_ego, y_ego, dx_ego, dy_ego, alpha_ego = vehicles["ego"].get_vehicle_in_rectangle(
        vehicles["ego"].xglob
    )
    ax.add_patch(patches.Rectangle((x_ego, y_ego), dx_ego, dy_ego, alpha_ego, color="red"))
    for name in list(vehicles):
        if name == "ego":
            ...
            ax.add_patch(patches.Rectangle((x_car, y_car), dx_car, dy_car, alpha_car, color="blue"))
